...
import { injected, walletconnect, coinbase} from "./Connector";
...
const walletIcon = {
  ...
  coinbase: 'https://app.sushi.com/images/wallets/coinbase.svg'
}
...
export default function ConnectWallet({handleClose}) {
  ...
  const activateWallet = async (currentConnector, connectorName) => {
    ...
    console.log({CC: currentConnector.connector})
    const  conn =  currentConnector.connector
    const Connector = typeof conn === 'function' ? await conn() : currentConnector
    await activate(Connector, undefined, true).catch(async (error) => {
      console.log(error);
        ...
        <Grid>
            ...
            {Object.keys(connectorsByName).map((name) => {
              ...
              if(isMobile && typeof conn !== 'function'){
                return (
                  <a className="meta-mask-wallet-container"
                ...
                >
                  <div style={styles.options}>
                    <img src={walletIcon.Injected} style={styles.icons} alt="wallet_icon" />
                    <div>
                      <Typography style={styles.walletName}>{walletName.Injected}</Typography>
                    </div>
                  </div>
                  <div
                    style={styles.loaderDiv}
                  >
                    {activating && <Spinner color={'black'} style={styles.loader} />}
                  </div>
                </a>
                )
              } else {
              return (
                <button className="meta-mask-wallet-container"
                  style={{
                    width: '100%',
                    borderColor: activating ? 'orange' : connected ? 'green' : 'unset',
                    cursor: disabled ? 'not-allowed' : 'pointer',
                ...
                >
                  <div style={styles.options}>
                    <img src={walletIcon[name]} style={styles.icons} alt="wallet_icon" />
                    <div>
                      <Typography style={styles.walletName}>{walletName[name]}</Typography>
                    </div>
                  </div>
                  <div
                    style={styles.loaderDiv}
